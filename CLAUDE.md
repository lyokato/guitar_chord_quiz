...
- バッキングコード: `BACKING_CHORDS`(A/B/C/D/E/G × m7・7 のワンコードバンプ。ペンタ等のアドリブ練習用)。構成音は`QUALITIES.tones`から導出。各コードは3素材: `midis`=中域 R-5-♭7-3(10度)/`high`=上位展開 ♭7-3-5-R(リフト用)/`bass`=ルート1オクターブ下。音色はパッド主体の2層: `padNote`(デチューン三角波2本+ノコギリ波、音価いっぱい持続してリリース。0.07×gain)+薄い`pluck`アタック(0.04×gain)、ベースは`bassNote`(サイン+三角)。12msストラムずらし
- バッキングの4小節フレーズ: 単調回避のためスケジューラ側で役割分け — 小節頭のヒットはベース+コード、奇数小節(bar%4が1,3)の最後のヒットは`high`ボイシング、4小節目(bar%4===3)の最後はゆっくりアルペジオ(60msずらし)。`rhythmSettings.backing`("none" | コード名)で選択、フィル小節でも通常通り鳴らす
