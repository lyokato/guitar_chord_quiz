...
- `BACKING_STYLES`: バッキング=ジャンル別ミニバンド(ペンタ等のアドリブ練習用)。実曲の定番構造に基づき、**キーはメジャー/マイナー両対応**("A"=メジャー, "Am"=マイナー。`BACKING_KEYS`=A/Am/B/Bm/C/Cm/D/Dm/E/Em/G/Gm): blues=12小節ブルース(メジャー=I7型 / マイナー=im7-ivm7-V7)+ブギーリフ+R-3-5-6ウォーキングベース / rock=メジャーI-IV-V-IV / マイナーi-♭VII-♭VI-♭VII+パワーコード刻み+8分ベース / funk=メジャーI9(JB型) / マイナーim7+16分スクラッチ+シンコペベース / house=メジャーIM7-vi7-ii7-V7 / マイナーi7-i7-♭VIM7-♭VII7+裏拍スタブ+オフビートベース / techno=1コード反復+キック位置を空けた16分ローリングベース(表強・裏中・他弱) / ballad=メジャーI-V-vi-IV / マイナーi-♭VI-♭III-♭VII+8分分散和音(pick)+ロングベース。ドラムパターンとは独立に選択(スウィングはドラムと同じ規則で自動適用)
- スタイル定義: `progression`=`{major:[…], minor:[…]}`(各小節 {deg: キーからの半音, qual}、qual: maj/min/dom7/m7/maj7/9)、`comp`/`bass`=`{bars:[小節譜面…], seq:[並び]}`(小節番号 % seq.length。`{major:…, minor:…}`のモード別指定も可)、`bed`=パッド土台の有無。hit=`{step, dur, voice, gain?, note?(bass用), idx?(pick用)}`。voice: `bass`/`pick`(単音アルペジオ)/`shell`/`mid`/`high`/`d5`・`d6`・`d7`(ダイアド。**d6の6度・ベースのnote:9はブギー/ドリアン定番の経過音として許容**)/`arp`。**ベースの`note:"3rd"`は各コードの長短3度に自動追従**(マイナーブルースのV7で♭3が衝突しないための仕組み。過去に検証で実バグ検出済み)。`rhythmSettings.backingStyle`+`backingKey`で選択、フィル小節でも通常通り鳴らす
- コード素材は`backingMaterials(pc, qual)`で導出+キャッシュ: `midis`=中域 / `high`=上位展開 / `shell`=ガイドトーン / `bass`=ルート1オクターブ下(9thはR-♭7-9-3のファンクボイシング)。ルートはE2〜D#3。音色: コード系はパッド主体(`padNote` 0.07+薄い`pluck` 0.04)、ダイアド系は歯切れ優先で`pluck`主体(0.10+0.03)、ベースは`bassNote`(サイン+三角)
- `padBed`: 途切れないパッドの土台(小節頭ごとに1小節+のりしろを予約、等長リニアクロスフェード0.4秒で連続音。デチューン三角波2本+フィルタLFO)。**「点の連続=打楽器的」を避ける要**。bed:trueのスタイル(rock/house/ballad)のみ
- 再生中は小節頭の拍イベントに現在のコード名を載せ、`#backing-chord`に表示(12小節ブルースでV7が来るのが見える=練習ガイド)
