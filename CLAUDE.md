...
- コントロールは3行。1行目は同寸の「ループ先頭へ戻る」「再生/一時停止トグル」を中央配置、2行目は「リセット」「速度select」「キーselect」、3行目は「カウントイン」「checkbox」「40〜240のBPM数値入力」を並べる。±10秒移動、現在位置から範囲端を設定するボタン、速度rangeは置かない
- カウントインは手動で再生を開始するときだけ鳴り、ループ復帰では鳴らない。4/4の2小節で、1小節目の1・3拍、2小節目の1・2・3・4拍へ既存`DRUM_SOUNDS`を予約し、8拍後の次小節頭から曲を再生する。実行中の再生ボタンはカウントイン停止として働く
