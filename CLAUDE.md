...
- コントロールは3領域。1つ目は同寸の「ループ先頭へ戻る」「再生/一時停止トグル」を中央配置、2つ目は「速度select」「キーselect」、3つ目は「カウント」「checkbox」「40〜240のBPM数値入力」と「lag」「0.0〜1.0秒の数値入力」を2段で並べる。範囲リセット、±10秒移動、現在位置から範囲端を設定するボタン、速度rangeは置かない
