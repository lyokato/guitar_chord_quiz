...
- シークバーの白いつまみは曲全体の再生位置、同じレール上のオレンジ色A/Bマーカーはループ境界を表す。A/Bはポインタードラッグまたは左右キー(Shift併用は1秒)で動かし、A ≤ B−3秒を常に保つ。3秒未満の曲は全体ループに固定する
- コントロールは1行。リセットは左端、速度selectは右端、中央には「Aへ戻る」「再生/一時停止トグル」を一組として絶対中央配置する。±10秒移動、現在位置からA/Bを設定するボタン、速度rangeは置かない
