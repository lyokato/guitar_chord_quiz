...
- `DRUM_PATTERNS`: 16分グリッド(拍あたり4ステップ)でパターン定義。`beats`=拍子, `swing`=シャッフル(8分裏を3連位置へ), `tracks`={楽器: ステップ配列}, `backing`=バッキングのコンピング譜面(下記。スウィングはドラムと同じ規則で自動適用)
- バッキングの鳴り: `padBed`=途切れないパッドの土台(小節頭ごとにR+♭7+3(10度)の3音を1小節+のりしろで予約し、等長リニアクロスフェード0.4秒で連続音にする。デチューン三角波2本+フィルタLFO)。**「点の連続=打楽器的」を避ける要**なので削らないこと。その上にコンピング譜面のヒットを重ねる
- バッキング譜面: `backing = { bars: [小節バリエーション…], seq: [並び] }`。小節番号 % seq.length で参照(4小節フレーズ)。hit=`{step, dur(ステップ数), voice, bass?, gain?}`。voice: `bass`=ベース単音 / `shell`=ガイドトーン / `mid`=中域 / `high`=上位展開 / `d5`・`d6`・`d7`=ルート+5度/6度/♭7のダイアド(シャッフルのブギーリフ用。**d6の6度はコードトーン外の定番経過音として許容**)/ `arp`=ゆっくりアルペジオ(60msずらし)
- バッキングコード: `BACKING_CHORDS`(A/B/C/D/E/G × m7・7 のワンコードバンプ。ペンタ等のアドリブ練習用)。構成音は`QUALITIES.tones`から導出。素材: `midis`=中域 R-5-♭7-3(10度)/`high`=上位展開 ♭7-3-5-R /`shell`=3度+7度 /`bass`=ルート1オクターブ下。音色: コード系はパッド主体(`padNote`=デチューン三角波2本+ノコギリ波で音価いっぱい持続 0.07 + 薄い`pluck` 0.04)、ダイアド系は刻みの歯切れ優先で`pluck`主体(0.10+パッド0.03)、ベースは`bassNote`(サイン+三角)。12msストラムずらし。`rhythmSettings.backing`("none" | コード名)で選択、フィル小節でも通常通り鳴らす
