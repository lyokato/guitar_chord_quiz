...
- 履歴は`lastPracticedAt`の降順で最大5曲。各行は容量・長さ・ループ範囲・速度を表示し、曲を再度開くと範囲と速度を復元する。現在の曲の見出し下には容量や「端末内」といった補足を表示しない。Blobが欠落したレコードは開く際に履歴から削除する
...
- シークバーの白いつまみは曲全体の再生位置、オレンジ色の無地マーカーはループ境界を表す。再生つまみの中心と境界線は、左右9pxを除いた`.timeline-scale`の同じ座標系へ置き、`currentTime >= loopEnd`でループする。マーカーはシーク操作領域より上、縦線だけをシークバーの下まで伸ばし、マーカーと履歴の画面上にA/Bの文字は出さない。各マーカーはポインタードラッグまたは左右キー(Shift併用は1秒)で動かし、開始 ≤ 終了−3秒を常に保つ。3秒未満の曲は全体ループに固定する
- コントロールは1行。リセットは左端、速度selectは右端、中央には同寸の「ループ先頭へ戻る」「再生/一時停止トグル」を一組として絶対中央配置する。±10秒移動、現在位置から範囲端を設定するボタン、速度rangeは置かない
