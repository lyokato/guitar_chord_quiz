...
- `BACKING_STYLES`: バッキング=ジャンル別ミニバンド(ペンタ等のアドリブ練習用)。実曲の定番構造に基づく: blues=12小節ブルース(I7×4→IV7×2→I7×2→V7→IV7→I7→V7)+ブギーリフ+R-3-5-6ウォーキングベース / rock=i-♭VII-♭VI-♭VIIヴァンプ+パワーコード刻み+8分ベース / funk=I9ワンコード+16分スクラッチ+シンコペベース / house=i7-i7-♭VIM7-♭VII7+裏拍スタブ+オフビートベース / techno=iマイナー反復+キック位置を空けた16分ローリングベース(表強・裏中・他弱) / ballad=i-♭VI-♭III-♭VII+8分分散和音(pick)+ロングベース。ドラムパターンとは独立に選択(スウィングはドラムと同じ規則で自動適用)
- スタイル定義: `progression`=[{deg: キーからの半音, qual}](小節ごと、qual: maj/min/dom7/m7/maj7/9)、`comp`/`bass`=`{bars:[小節譜面…], seq:[並び]}`(小節番号 % seq.length)、`bed`=パッド土台の有無。hit=`{step, dur, voice, gain?, note?(bass用), idx?(pick用)}`。voice: `bass`/`pick`(単音アルペジオ)/`shell`/`mid`/`high`/`d5`・`d6`・`d7`(ダイアド。**d6の6度・ベースのnote:9はブギー定番の経過音として許容**)/`arp`。キーは`BACKING_KEYS`(A/B/C/D/E/G)、`rhythmSettings.backingStyle`+`backingKey`で選択、フィル小節でも通常通り鳴らす
